...
df1_Voted_NotVoted <- anes %>%mutate(Voter = case_when(V202109x == 1 ~ 'voted' , 
                                        V202109x == 0 ~ 'not voted'   
                                         ...
                                         !V201228 %in% 1:2 & V201230 == 3 ~ "Republican"
...
y2 = c(1.76, 11.45 , 9.25 ,11.89, 9.25 , 3.96, 2.20 , 10.57 , 0.88 , 0.0 , 2.64, 0, 1.76, 1.32 , 7.9 , 16.74 , 1.32, 0, 0, 0, 4.4 , 2.64,
       1.42,6.10,2.41,14.05,13.03,1.62,3.05,7.53,1.62,0.20,2.64,1.22,1.42,0.81,7.12,20.36,0.20,0,0,0,5.90,1.62)
...
y2 = c(88.55,7.58,2.79,0.63,0.42 , 89.48, 6.5 , 2.41 , 0.95 , 0.95)
...
```{r}
str(as.numeric(as.factor(table$Party)))
```
